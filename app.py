...
# Load the new dataset (SMS Spam Collection)
url = "https://raw.githubusercontent.com/justmarkham/DAT8/master/data/sms.tsv"
response = requests.get(url)
data = response.content.decode('utf-8')

# Save the data to a file
with open("dataset.tsv", "w", encoding="utf-8") as file:
    file.write(data)

# Load the dataset from file
df = pd.read_csv('dataset.tsv', sep='\t', header=None)
text = ' '.join(df[1].tolist())
...
def generate_text(model, start_text, max_length=50):
    ...
    for _ in range(max_length):
        ...
        valid_indices = [i for i, word in index2word.items() if not word.isdigit()]
        p = p[valid_indices]
        p /= p.sum()

        word_index = np.random.choice(valid_indices, p=p)
